...
class Notes extends Component {
  ...
  noteLongPress = (index) => {
    Alert.alert("Do u want to DELETE this note?", "No possibility to restore", [
      {
        text: "Cancel",
      ...
      },
      {
        text: "Delete",
        style: "destructive",
        onPress: () => this._deleteNote(index),
      },
    ]);
  };

  _deleteNote = async (index) => {
    let keysArr = this.state.keysArr;
	let colorsArr = this.state.colors;
	let chosenColor = '';

	let filteredKeysArr = keysArr.filter((element, i) => {
      if (i != index) return element;
    });
	
	let filteredColorsArr = colorsArr.filter((element, i) => {
		if(i != index) return element;
		else chosenColor = element;
	});
	filteredColorsArr.push(chosenColor);
	this.setState({colors: filteredColorsArr});
    
	await SecureStore.setItemAsync("keysArr", JSON.stringify(filteredKeysArr));
    this.getNotes();
  };
